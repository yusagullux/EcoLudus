...
|------|--------|--------|----------|---------|
| **2.1 Carbon Tracking Integration** | ⭐⭐⭐ | High | Climatiq (200/mo free) | ✅ IMPLEMENTED - Added `lib/carbon-calc.ts`, DB-backed `carbon_cache`, and `/api/quests/complete`. Dashboard completions now validate quests server-side, cache carbon values, update profiles, and write `mission_logs` with carbon source metadata. Production needs `CLIMATIQ_API_KEY`; without it, values are transparently sourced from `quests.json` as `quest_catalog`. |
